...
import (
	...
	"github.com/nxsre/criproxy/pkg/runtimeapis"
...
func NewFakeCriServer110(journal Journal, streamUrl string) FakeCriServer {
	...
	v1_15.RegisterRuntimeServiceServer(s.server, s)
	v1_15.RegisterImageServiceServer(s.server, s)
	return s
}

func (s *FakeCriServer110) SetFakeContainerStats(containerId, containerName, imageFsUUID string) interface{} {
	r := MakeFakeContainerStats110(containerId, &v1_15.ContainerMetadata{
		Name: "container1",
	}, imageFsUUID)
	s.FakeRuntimeServer110.SetFakeContainerStats([]*v1_15.ContainerStats{r})
...
func (s *FakeCriServer110) SetFakeFilesystemUsage(imageFsUUID string) interface{} {
	...
	s.FakeImageServer110.SetFakeFilesystemUsage([]*v1_15.FilesystemUsage{r})
	out, err := runtimeapis.Downgrade(r)
	if err != nil {
		log.Fatalf("Downgrade %T: %v", r, err)
	}
